...
include("function.php"); // connection to db
error_reporting(0);
session_start();
...
function insert()
{
    ...
    //parameter = 'uid='+document.getElementById('uid').value ; // set = to the `data` you want to update the database with. - FROM THE FORM
    parameter1 = 'pid='+document.getElementById('pid').value;// set = to the `id` WHERE you want to replace the data - FROM THE FORM
    parameter2 = 'price='+document.getElementById('price').value ; // set = to the `data` you want to update the database with. - FROM THE FORM
    parameter3 = 'q='+document.getElementById('quantity').value;
    var postArray = new Array(4); // array to hold two parameters from the form. Adjust as needed
    postArray[0] = parameter1; // add first parameter to the array
    postArray[1] = parameter2;
    postArray[2] = parameter3;
    //postArray[3] = parameter4;
    xmlhttp.open('POST', 'insert.inc.php', true);
    xmlhttp.setRequestHeader('Content-type','application/x-www-form-urlencoded');
    xmlhttp.send(postArray.join("&") );
}
</script>
                    ...
                    <div class="sidebar-wrap">
                           <div class="widget clearfix">
                            ...
                            <div class="widget-body">
                                <ul class="tags">
                                <li> <form action="food.php" method ="post">
...
$stmt = $db->prepare("select * from food_category ");
$stmt->execute();
...
?>
                                    <li><form action="food.php" method ="post">
                                <input type="hidden" name="c" value="<?php echo $cat['id'] ;?>">
<button type="submit"  class="tag">
                                    <?php echo $cat['name'] ;?>
                                    </button> </form></li>
                                 <?php }}?>
                         ...
                         <div class="widget widget-cart">
                                ...
                                <div class="order-row bg-white" id=data>
...
<?php  
if(isset($_POST['c']) || isset($_POST['query'])){
if($_POST['c'] == 'all'){
$stmt = $db->prepare("select * from dishe");
}elseif(isset($_POST['query'])){

$srh = $_POST['search'];
$stmt = $db->prepare("select * from dishe where title like '%$srh%' or slogan like '%$srh%'");
}
else{
$wh = $_POST['c'];
$stmt = $db->prepare("select * from dishe where Category = '$wh'");
}
}
else{// display values and item of food/dishes
...
<!--<form method="post" action='delete.php?action=addcart'>-->
<form method="post" >
...
<input  type="hidden" name="id" id="pid"  value="<?php echo $product['d_id']; ?>"  />
<input  type="number" name="quantity" id="quantity" style="margin-left:30px; width:30px;" value="1"  />
<input type="submit" class="btn theme-btn" name="cart" style="margin-left:40px;" onClick="insert();" value="Add to cart" />
</div>
</form>
                                    </div>
                                    <!-- end:row -->
